defmodule Homework.Merchants do
  ...
  def get_merchant_transactions(id) do
    record = Homework.Merchants.get_merchant!(id) |> Homework.Repo.preload(:transactions)
    {:ok, record.transactions}
  end

  def find_merchants_by_name(name) do
    merchants = Homework.Repo.query("SELECT name FROM merchants WHERE SIMILARITY(name, $1) > 0.1 ;", ["#{name}"])
    {:ok, %{merchants: List.flatten(elem(merchants, 1).rows)}}
  end
end
